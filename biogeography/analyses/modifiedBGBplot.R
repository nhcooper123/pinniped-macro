plot_BioGeoBEARS_mod <-
function (results_object, analysis_titletxt = NULL, addl_params = list(),
          ...
          tr = NULL, tipranges = NULL, if_ties = "takefirst", tip_statecex = 0.7, 
          ...
          tip_stateadj = 0.5, tip_stateshape = 16) 
{
  ...
  if (skiptree != TRUE) {
    plot.phylo(tr_pruningwise, x.lim = xlims, y.lim = ylims, show.tip.label = show.tip.label, 
         label.offset = label.offset, cex = tipcex, no.margin = no.margin, 
         root.edge = root.edge, underscore = FALSE, font = 3)
    
    #--------------------------------------------------------
    # NC: Removed this as plot.phylo() allows plotting tip labels
    # without _ and italics, which is not possible using tiplabels()
    #----------------------------------------------------------
   # if (show.tip.label == TRUE) {
     ...
     #             pos = 4, offset = label.offset, col = tipcol, font = 3)
     # }
   # }
  ...
  if (plotwhat == "text") {
    par(fg = tmp_fg)
    if (skiplabels == FALSE) {
      tiplabels(pch = tip_stateshape, cex = tip_statecex, bg = cols_byNode)
      nodelabels(text = MLstates[nodes], node = nodes,
                 bg = cols_byNode[nodes], cex = statecex, frame = "circle")
      #----------------------------------------------------------
      # NC: Don't label tips
      #tiplabels(text = MLstates[tips], tip = tips, bg = cols_byNode[tips], 
                #cex = statecex, frame = "none")
  ...
  if (plotwhat == "pie") {
    ...
    if (skiplabels == FALSE) {
      #---------------------------------
      # NC: Adding tip states as points here
      #---------------------------------
      if (tipboxes_TF == TRUE) {
        tiplabels(pch = tip_stateshape, cex = tip_statecex, bg = cols_byNode,
                  adj = tip_stateadj)
