...
\title{Supplementary Materials from: \textit{Charting the course of pinniped evolution: insights from molecular phylogeny and fossil record integration.}} 
...
\input{figures/table-genbank.tex}
...
\subsubsection{Median tree topology}

The final MRP matrix contained 129 Pan-Pinnipedia taxa (36 extant species including the recently extinct Caribbean monk seal \textit{Neomonachus tropicalis} and Japanese sea lion \textit{Zalophus japonicus}, and 93 fossil taxa of which 17 are specimen-level OTUs), which is approximately 93\% of all recognised pinniped species in the PBDB (https://paleobiodb.org, accessed June 27, 2023). Here, we provide a general overview of the topology of the median tree from the BEAST posterior distribution of time-scaled trees (Figure 1), which excludes four fossil taxa that lacked sufficiently constrained stratigraphic data for inclusion. All major pinniped clades were recovered as monophyletic (Desmatophocidae, Odobenidae, Otariidae, Phocidae). 

Pan-Pinnipedia originated in the late Early Oligocene, $\sim$29 Ma, with the divergence of \textit{Potamotherium} and \textit{Puijila}, which are sister to all other pinnipeds. \textit{Enaliarctos} is paraphyletic, and, along with \textit{Pteronarctos} and \textit{Pinnarctidion}, are recovered as stem-pinnipeds (Pinnipedimorpha). Crown pinnipeds (Otarioidea $+$ Phocoidea) originated in the latest Oligocene 24.68 Ma (95\% Highest Posterior Density interval [HPD] = 23.38 - 25.97 Ma).

Otariidae and Odobenidae (Otarioidea) are recovered as sister lineages that diverged in the Early Miocene (20.1 Ma, 95\% HPD = 17.97 - 22.42 Ma). The topology for Pan-Otariidae broadly follows recent studies, with \textit{Eotaria} as the oldest lineage and sister to all other taxa, followed by a paraphyletic \textit{Thalassoleon} and \textit{Callorhinus}. Crown otariids fall into a previously noted northern-southern hemisphere split, with the exception of \textit{Neophoca palatina}. A key difference in our crown otariid topology relative to previous work is the position of the Early Pliocene taxon \textit{Hydrarctos lomasiensis}, which is nested within \textit{Arctocephalus}. This results in relatively old divergence time estimates for crown Otariidae (9.64 Ma, 95\% HPD = 7.86 - 11.84 Ma).

Within Odobenidae, we find that an unnamed taxon (LACM 135920) is sister to all other walruses followed by a clade consisting of \textit{Prototaria} spp. and \textit{Proneotherium repenningi}, taxa which have tended to form the earliest-diverging lineages in a succession of basal odobenids in other studies. Within Neodobenia, we find a monophyletic Dusignathinae and Odobeninae (and a monophyletic Odobenini), with the latter subfamily including \textit{Pliopedia pacifica}.

We resolved Desmatophocidae as sister to Phocidae, forming the Phocoidea. \textit{Eodesmus} is recovered as sister to an unnamed taxon (USNM 335445) and this clade is sister to all other desmatophocids. We do not find a monophyletic Desmatophocinae. Within the Phocidae, Devinophocinae, Phocinae, and Monachinae are monophyletic, as in most previous work. However, recovery of \textit{Noriphoca gaudini} as a stem monachine drives the origin of the crown group back to the earliest Miocene (22.05 Ma, 95\% HPD = 21.08 - 23.23 Ma). The phocine stem consists of \textit{Leptophoca proxima}, \textit{Kawas benegasorum}, \textit{Nanophoca vitulinoides}, and a paraphyletic \textit{Praepusa}. Crown Phocinae originated at 13.51 Ma (95\% HPD = 10.9 - 15.77 Ma) with the divergence of \textit{Erignathus} from all other taxa, but median divergence times for all other crown nodes are younger than 8 Ma. Stem monachines include \textit{Frisiphoca} spp., the ‘Waipunga’ specimen of Boessenecker and Churchill (\citeyear{boessenecker2016origin}) which has previously been identified as an elephant seal, \textit{Noriphoca gaudini}, \textit{Properiptychus argentinus}, and \textit{Australophoca changorum}. Within crown Monachinae, which originated $\sim$ 10.87 Ma, the relationships between the tribes Monachini, Miroungini, and Lobodontini reflect the consensus seen in previous studies. The immediate sister lineage to crown-Monachinae is an unnamed taxon from the Pliocene Lee Creek Formation of North Carolina, with a Late Miocene- Early Pliocene clade comprising \textit{Homiphoca}, \textit{Acrophoca}, \textit{Piscophoca}, \textit{Sarcodectes}, and \textit{Hadrokirus} forming the next outgroup. 
...
Our new metatree for Pan-Pinnipedia represents the most densely sampled phylogeny of the clade to date, and the tree presents some new interpretations of pinniped evolutionary history. Our median age estimate for the clade itself of $\sim$29 Ma is younger than that of several previous studies \citep{arnason2006pinniped,higdon2007phylogeny,fulton2010multiple2,paterson2020total} but is similar to that of Rule et al. (\citeyear{rule2020first}). 
Similar to several recent studies, we recognise the superfamily Phocoidea (Desmatophocidae $+$ Phocidae).

Within Otariidae, the New Zealand fossil taxon Neophoca palatina was found to be sister of crown otariids (except \textit{Callorhinus ursinus}) rather than forming a clade with \textit{N. cinerea}, a result similar to Churchill et al. (\citeyear{churchill2014colonization}). 
This places \textit{N. palatina} in an otherwise exclusively northern hemisphere group, suggesting more than one southern hemisphere dispersal event by otariids. 
However, the study by Churchill and Boessenecker (\citeyear{churchill2016taxonomy}) found morphometric evidence for \textit{N. palatina} and \textit{N. cinerea} to belong to the same genus (thereby retaining a single southern hemisphere dispersal) and cautioned that the qualitative morphological characters used in previous studies of otariid phylogeny may not properly capture differences in cranial shape between taxa. 
Future studies should aim to develop new characters to resolve this and improve otariid morphological phylogenetics.
Our other notable finding within otariids was the position of \textit{Hydrarctos lomasiensis} nested within \textit{Arctocephalus}, a result not known from any other published phylogeny. 
It was however found to be sister to \textit{Arctocephalus} in Churchill et al. (\citeyear{churchill2014colonization}; Fig 5B) when poorly known fossil taxa were excluded from the analyses, and when their morphological data is analysed separately it does fall within \textit{Arctocephalus}. 
\textit{Hydrarctos} was initially named as a species of \textit{Arctocephalus} (subgenus \textit{Hydrarctos}) by Muizon (\citeyear{muizon1978arctocephalus}), who proposed it was an early diverging member of the genus that had evolved towards being more specialised. 
It was then assigned full generic rank by Berta and Dem\'{e}r\'{e} (\citeyear{berta1986callorhinus}), who found it to be outside crown group otariids. 
The knock on effect on our topology is that the divergence dates for \textit{Arctocephalus} (and other crown taxa) are much older than suggested by genomic data, producing relatively long ghost lineages. Our results suggest \textit{Arctocephalus} (including \textit{Hydrarctos}) diverged $\sim$6 Ma whereas Lopes et al. (\citeyear{lopes2021phylogenomic}) give a value of $\sim$2.8 Ma for \textit{Arctocephalus sensu stricto}, implying an earlier dispersal to the southern hemisphere than has previously been supposed. 
If novel morphological characters are developed for otariid phylogenetic inference, as proposed above, the relationship between \textit{Arctocephalus} and \textit{Hydrarctos} would be a useful example to test them with.

Within Phocidae, our placement of \textit{Noriphoca gaudini} as a stem monachine follows that of Dewaele et al. (\citeyear{dewaele2018critical}).
 The consequence of having \textit{N. gaudini} as part of crown Phocidae is that the divergence dates for the crown are pushed back, {}becoming considerably older than most other estimates from both molecular and morphological/combined evidence studies e.g. \citep[e.g.][]{higdon2007phylogeny,fulton2010multiple2,scheel2014biogeography,paterson2020total,rule2020first} but see \citep{arnason2006pinniped,dewaele2018critical}. 
 Given the earliest diverging phocids in our study, \textit{Devinophoca} spp. split from other phocids $\sim$1 Ma prior to divergence of the two crown subfamilies, the diversification of phocids appears to have occurred rapidly. 
 We additionally find \textit{Frisiphoca} spp. to be stem monachines, rather than stem phocines as found by Dewaele et al. (\citeyear{dewaele2018critical}). 
 The material however, is fragmentary, and this position could change with the addition of more complete material. 
 Another novel result was our placement of cf. Miroungini indet. CD 35 (also known as the Waipunga specimen) as a stem monachine rather than as a stem Miroungini, as had been proposed by Boessenecker and Churchill (\citeyear{boessenecker2016origin}) and supported by Rule et al. (\citeyear{rule2020first}). 
 Despite several shared features with \textit{Mirounga} (simplified triple-cusped upper postcanine teeth with smooth enamel and lacking cingulum, reduced M1, and possession of a probable prenarial shelf), the specimen also retains several features considered archaic for phocids (double-rooted P2–4, apparent retention of parastyle and metacone, and probable small adult body size).
  These features, in combination with the fragmentary nature of the specimen, likely give rise to its placement in our topology.   

The divergence ages of the phocids also raise some questions about the higher level taxonomy of the clade. 
The monophyly of the two subfamilies, Monachinae and Phocinae is well supported by morphological \citep{berta1994pinniped,rule2020first,orp1996morphological,rule2020first} and molecular data \citep{davis2004phylogeny,arnason2006pinniped,higdon2007phylogeny,dasmahapatra2009pinniped,fulton2010multiple2,scheel2014biogeography}.
Given this, and the fact that their earliest divergences are at least as old as those of the Otariidae and Odobenidae (Figure 1), an argument could be put forward for the two phocid subfamilies to be accorded their own family ranking (e.g. Phocidae and Monachidae). 
We do not officially propose this here, but recommend that the idea be given further consideration in future studies.
...
\input{figures/table-unlikely.tex}
...
\input{figures/table-biogeo-results.tex}

% table of unlikely results
\input{figures/table-unlikely-fossil-extant.tex}
